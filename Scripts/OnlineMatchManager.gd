...
func _setup_cameras():
	...
	for cam in all_cameras:
		cam.current = false
		# We don't want to disable the node itself if it's our camera, 
		# so we'll handle activation separately.
	...
	# 3. Handle other players (optional: hide their cameras/HUDs if they exist)
	for i in range(4):
		if i == role: continue
		...
		if other_p:
			...
			if other_cam:
				...
				# Hide UI components of other players
